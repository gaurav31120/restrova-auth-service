...
2 docker volume create mernpgdata

3 docker volume ls

// old version below 18
// The issue was: PostgreSQL Docker image version 18+ changed the expected mount point from /var/lib/postgresql/data to /var/lib/postgresql.

4 docker run --rm --name mernpgdata-container -e POSTGRES_USER=root -e POSTGRES_PASSWORD=root -v mernpgdata:/var/lib/postgresql/data -p 5432:5432 -d postgres

// New version 18+
4 docker run --rm --name mernpgdata-container -e POSTGRES_USER=root -e POSTGRES_PASSWORD=root -v mernpgdata:/var/lib/postgresql -p 5432:5432 -d postgres

5 Command to stop the container

docker stop id_name

6 create a connection in dbGate